...
{
  public class AtividadeOnboardingOnboardingRepository(AppDbContext context) : BaseRepository(context), IAtividadeOnboardingRepository
  {
    public async Task<IEnumerable<AtividadeOnboarding>> ListAsync() 
      => await _context.AtividadesOnboardings.AsNoTracking().ToListAsync();

    public async Task<AtividadeOnboarding?> GetByIdAsync(int id)
